...
def rent_machine(machine_id,
    ...
    # 
    cmd = f"vastai create instance {machine_id} --image {image} --disk {storage} --ssh --direct --onstart {start_script} --raw"
...
@click.option("--vastaikey",type=str,default="~/.ssh/id_rsa_vast.ai",help="vastai key to use")
...
"""
    ...
    for instance in json_res:
        ...
        if "ports" in instance:
            ...
            sshmp = ssh_template%(str(instance_id)+"tun",public_ipaddr,ports,vastaikey)
...
@cli.command(help="""rent cheapest instance based on search\n Example:\n\n \
gpu_name==RTX_3090 num_gpus==1 rented=False verified=True
             """)
@click.option("--query",default="gpu_name==RTX_3090 num_gpus==1 rented=False verified=True",help="query to search for")
...
@click.option("--image",type=str,default="pytorch/pytorch:latest",help="image to use")
def rent(query,start_script,storage,image):
    insts = list_instance_matching_format(query+f" disk_space>{int(storage)*2}")
    print("found %s instances"%len(insts))

    selected = insts[0]

    print("selected instance:",selected["id"],selected["gpu_name"],selected["num_gpus"])

    renting_response = rent_machine(selected["id"],
                storage=storage,
                image=image,
...
@cli.command(help="""list available instances matching query\n Example:\n\n \
...
def query(query):
    insts = list_instance_matching_format(query)
