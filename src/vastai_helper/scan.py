...
p:ArgumentParser = vast.parser.parser
p.register
...
def list_instance_matching_format(vastai_exe,query):
    ...
    print("used q",query)
    cmd = f"{vastai_exe} search offers '{query}' --raw "
    print("used command",cmd)
    res = subprocess.check_output(cmd, shell=True)
    insts = json.loads(res)
    return insts
    
def rent_machine(vastaiexe,machine_id,
                storage=30.28,
...
@cli.command(help="""generate pretty ssh configuration to use with existing instances""")
@click.option("--vastaiexe",type=str,default="vastai",help="vastaiexe to use")
@click.option("--vastaikey",type=str,default="~/.ssh/id_rsa_vast.ai",help="vastai sshkey to use")
...
def rent(query,start_script,storage,image,vastaiexe):
    insts = list_instance_matching_format(vastaiexe,query+f" disk_space>{int(storage)*2}")
    ...
    renting_response = rent_machine(vastaiexe,selected["id"],
                storage=storage,
                image=image,
